...
from api.test.test_payloads import *
...
class TestAuthHandler(TestCase):
    ...
    def test_login_success(self):
        response = self._do_login(login_payload())
    ...
    def test_otp_success(self):
        payload = auth_signup_payload(otp=True, rand=get_random_string(4))
        ...
        session = self.client.post(
            "{}/tfa".format(self.resource),
            json={"token": pre_login_result["data"]["token"], "code": otp_code},
        )
    ...
    def test_otp_failed(self):
        ...
        self.assertTrue(pre_login_result["code"] == 1)
        session = self.client.post(
            "{}/tfa".format(self.resource), json={"token": otp_code, "code": otp_code}
        )
        result = json.loads(session.data.decode("utf-8"))
        self.assert400(session)
    ...
    def test_signup_success(self):
        payload = auth_signup_payload(rand=get_random_string(8))
        response = self.client.put(self.resource, json=payload)
        result = json.loads(response.data.decode("utf-8"))
        self.assert200(response)
        self.assertTrue(result["code"] == 0)
    ...
    def test_edit_success(self):
        payload = auth_signup_payload(rand=get_random_string(6))
        self.client.put(self.resource, json=payload)
        ...
        edit_response = self.client.patch(
            self.resource,
            json=payload,
            headers={"Authorization": "Bearer {}".format(credentials["data"]["token"])},
        )
    ...
    def test_delete_success(self):
        ...
        delete_response = self.client.delete(
            self.resource,
            headers={"Authorization": "Bearer {}".format(result["data"]["token"])},
        )
